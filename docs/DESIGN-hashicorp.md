...
3. **Monochrome first.** Cards, banners, pills, icons and avatars are charcoal/gray/white only. Chromatic color is reserved for status (`--cat-green` success, red errors) and the amber brand accent. Per-category rainbow colors are deliberately gone; do not reintroduce them.
4. **Humanized voice:** warm copy, amber selection highlight, amber error-page CTAs. The system should read as a friendly PH marketplace built on engineered foundations - not a developer-infrastructure site.
5. Everything else in this analysis (black canvas, charcoal surfaces, 1px hairlines, 8px CTA corners, tight display / relaxed body line-heights, eyebrow pattern) applies as written.
